...
public class Board {
    ...
    public void updateOpenSpaces() {
        ...
        for (int i = 0; i < gameBoard.length; i++){
            for (int j = 0; j < gameBoard[i].length; j++){
                if (gameBoard[i][j] ==0){
                    BoardSpot emptyspace = new BoardSpot(i, j);
                    openSpaces.add(emptyspace);
                }
            }
        }
    }
    ...
    public void addRandomTile() {
        // WRITE YOUR CODE HERE
        int rand1 = StdRandom.uniform(0, openSpaces.size());
        double rand2 = StdRandom.uniform(); 
        if (rand2 < 0.1) {
            gameBoard[openSpaces.get(rand1).getRow()][openSpaces.get(rand1).getCol()] = 4;
        }
        else{
            gameBoard[openSpaces.get(rand1).getRow()][openSpaces.get(rand1).getCol()] = 2;
        }
    ...
    public void swipeLeft() {
        ...
        for (int i = 0; i < gameBoard.length; i++){
            for (int j = 1; j < gameBoard[i].length; j++){
                int x = j; 
                while (x > 0){
                    if ((gameBoard[i][x-1] == 0) && (gameBoard[i][x] != 0)){ // (gameBoard[i][x] != 0)??
                        gameBoard[i][x-1] = gameBoard [i][x];
                        gameBoard[i][x] = 0;
                    }
                    x--;
    ...
    /**
     * Find and merge all identical left pairs in the board. Ex: "2 2 2 2" will become "4 0 4 0".
